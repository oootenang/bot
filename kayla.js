...
//Whm Setting
let hostwhm = "portal.meixgan.cloud" //Host server whm contoh : login.ditzzsenpai.wtf
let usrwhm = "ikhsan" //username whm
let passwhm = "#@#server#@#" //Password whm
let tokenwhm = "" // caranya => https://www.eukhost.com/kb/how-to-generate-an-api-token-using-whm/
let ipsrv = "52.139.174.158" //ip server whm

//Biarin Jgn Di ubah
let authWhm = {headers: {Authorization: `WHM ${usrwhm}:${passwhm}`}}
...
𝘚𝘢𝘺𝘢 Ikhsan 𝘉𝘰𝘵 𝘞𝘩𝘢𝘵𝘴𝘈𝘱𝘱 𝘠𝘢𝘯𝘨 𝘉𝘪𝘴𝘢 𝘔𝘦𝘮𝘣𝘢𝘯𝘵𝘶 𝘈𝘯𝘥𝘢 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘚𝘵𝘪𝘤𝘬𝘦𝘳, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘛𝘪𝘬𝘵𝘰𝘬, 𝘋𝘰𝘸𝘯𝘭𝘰𝘢𝘥 𝘝𝘪𝘥𝘦𝘰/𝘈𝘶𝘥𝘪𝘰 𝘠𝘰𝘶𝘵𝘶𝘣𝘦, 𝘔𝘦𝘮𝘣𝘶𝘢𝘵 𝘓𝘰𝘨𝘰 𝘛𝘦𝘹𝘵 𝘗𝘳𝘰/𝘗𝘩𝘰𝘵𝘰𝘰𝘹𝘺/𝘌𝘱𝘩𝘰𝘵𝘰360, 𝘋𝘢𝘯 𝘓𝘢𝘪𝘯-𝘭𝘢𝘪𝘯.
...
"mediaType": 1, 
"thumbnail": ppnyauser,
"mediaUrl": 'https://chat.whatsapp.com/GpwaG5nvU2yCitgJ7c8o8f',
"sourceUrl": 'https://chat.whatsapp.com/GpwaG5nvU2yCitgJ7c8o8f'
}
}
})
break
...
`
reply(result4)
if (!m.isGroup) return reply('*document udah dikirim ke chat pribadi bot.*')
kayla.sendMessage(sender, {document:{url:baby1[0].link}, fileName:baby1[0].nama, mimetype: baby1[0].mime}, {quoted:m}).catch ((err) => reply('Gagal saat mendownload File'))
break
...
await kayla.groupUpdateSubject(from, q)
.then( res => {
reply(`Sukses`)
...
if (!isBotAdmins) return reply(mess.botAdmin)
if (!q) return reply(`Kirim perintah ${command} teks`)
await kayla.groupUpdateDescription(from, q)
.then( res => {
reply(`Sukses`)
}).catch(() => reply(mess.error.api))
break
...
kayla.sendMessage(from, { document: { url: url }, fileName: filename, mimetype: 'application/zip' }, { quoted:m }).catch((err) => reply('Maaf link github yang kamu berikan di private, dan tidak bisa di jadikan file'))
}
break
...
if (!isAdmins && !itsMeKayla) return reply(mess.admin)
if (!isBotAdmins) return reply(mess.botAdmin)
await kayla.groupRevokeInvite(from)
.then( res => {
...
kayla.sendMessage(from, { text: teks_tagall, mentions: participants.map(a => a.id) }, { quoted:m })
break
...
case 'createakun':
if (!itsMeKayla) return reply(mess.owner)
if (!q) return reply(`*CREATE ACCOUNT*\nExample:\n#${command} domain|package\n\nContoh:\n#${command} ikhsanhost.com|Whm Mini`)
...
let e = String(err)
kayla.sendMessage("6285756448056@s.whatsapp.net", { text: "Hallo Owner Sepertinya Ada Yang Error Harap Di Perbaiki " + util.format(e), 
contextInfo:{
forwardingScore: 5, 
isForwarded: true
}})
